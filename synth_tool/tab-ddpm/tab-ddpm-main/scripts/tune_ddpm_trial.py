...
def objective(trial):
    ...
    weight_decay = 0.0    
    batch_size = trial.suggest_categorical('batch_size', [256]) # Fixed to 256 for speed
    
    # --- SPEED CHANGE: Reduced steps significantly for debug ---
    steps = trial.suggest_categorical('steps', [500]) 
    
    gaussian_loss_type = 'mse'
    
    # --- SPEED CHANGE: Reduced timesteps to 100 (faster sampling) ---
    num_timesteps = trial.suggest_categorical('num_timesteps', [100])
    
    # --- SPEED CHANGE: generate fewer samples for debug ---
    # This ensures we don't generate massive amounts of data during a test run
    num_samples = 500 
    ...
    # --- SPEED CHANGE: Evaluate on 1 seed instead of 5 ---
    n_datasets = 1
    score = 0.0
...
# --- SPEED CHANGE: Only 2 trials to verify the loop works ---
...
if args.eval_seeds:
    ...
    # --- SPEED CHANGE: Reduced eval seeds at the end from 10 to 1 ---
    subprocess.run(['python3.9', f'{eval_seeds}', '--config', f'{best_exp}', '1', "ddpm", eval_type, args.eval_model, '5'], check=True)